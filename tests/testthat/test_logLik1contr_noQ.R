...
markerDO = "D3S1358" #marker to perform "incident" on
...
test_that("check maximum likelihood Hp:", {
  ...
  expect(round(valid$ProbObs,s0),c(0.665,0.957,0.665,0.242,0.253,0.29,0.052,0.039,0.003,0.55,0.857,0.537,0.323,0.463,0.423,0.293,0.472,0.781,0.336,0.484,0.323,0.456,0.759,0.992,0.241))
})

test_that("check maximum likelihood Hd (unrelated):", {
  cond=0
  mle = contLikMLE(nC=NOC,samples=dat$samples,popFreq=dat$popFreq,refData=dat$refData,condOrder=cond,knownRef = 1,xi=NULL,prC=pC, threshT=AT,fst=fst,lambda=lam,xiFW=NULL, seed=seed0,steptol=steptol0,nDone=nDone0)
  thhat=mle$fit$thetahat2 #obtain maximum likelihood estimates
  ...
  #paste0(round(thhat,s0),collapse = ",")
  expect(round(thhat,s0),c( 1,755.989,0.166,0.11,0.044) )
  expect(round(mle$fit$loglik,s0), -167.322) 
  
  #CHECK Cumulative probs    
  #paste0(round(valid$ProbObs,s0),collapse = ",")
  valid = validMLEmodel(mle,kit=kit0,createplot=FALSE,verbose = FALSE)
  expect(round(valid$ProbObs,s0),c(0.665,0.957,0.665,0.242,0.253,0.29,0.051,0.039,0.003,0.55,0.857,0.541,0.323,0.463,0.423,0.293,0.472,0.781,0.336,0.484,0.323,0.457,0.759,0.992,0.241))
