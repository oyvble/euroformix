...
- Contains ordinary implementation of EFM:
	Used for Model validation. Genotypes are traversed only once.
...
void loglikGamma_cumprob(double *pvalVEC, double *maxY, int *nJointGenos, int *NOC, int *NOK, int *nKnowns, 
	...
	for(int locind=0; locind< *nMarkers; locind++) {	//for each marker:
		...
		for (rr = 0; rr < nReps0; rr++) { //traverse each replicate		
			for (aa = 0; aa < nAlleles; aa++) { //traverse each observed alleles (indices), also the Q-allele. Potential not necessary!
				...
				if( peaksLong[SI_nAllelesReps0 + aaind] < AT0 ) { //no PH observation to consider
						...
						for (a = 0; a < nAlleles; a++) { //traverse each observed alleles (also consider the last allele since Q-allele may have PH last allee)				
							...
							if( aind!=aaind ) { //proceed as before if different index							
								...
								if(shapev2[a]>smalltol) { //If contribution and PH>0  					//CONTRIBUTION SET (A)
									...
									logevidProb += log( pgamma(peak, shapev2[a],scale0[repID], 1, 0) - pgamma(AT0-1, shapev2[a],scale0[repID], 1, 0));
